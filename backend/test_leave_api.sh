...
test_leave_requests() {
    ...
    make_request "POST" "$BASE_URL/api/leave/requests/" \
        '{"leave_type": "annual_leave", "start_date": "2024-12-01", "end_date": "2024-12-05", "reason": "API Test Leave"}' \
        "Create Leave Request"

    # Get specific leave request (assuming slug exists)
    make_request "GET" "$BASE_URL/api/leave/requests/leave-test-2024-12-01/" "" "Get Leave Request Details"

    # Update leave request
    make_request "PATCH" "$BASE_URL/api/leave/requests/leave-test-2024-12-01/" \
        '{"reason": "Updated API Test Leave"}' \
        "Update Leave Request"

    # Get workflow status
    make_request "GET" "$BASE_URL/api/leave/requests/leave-test-2024-12-01/workflow_status/" "" "Get Workflow Status"

    # Approve at level (if user has permissions)
    make_request "POST" "$BASE_URL/api/leave/requests/leave-test-2024-12-01/approve_level/" \
        '{"notes": "Approved via API test"}' \
        "Approve Leave Request at Level"

    # Cancel leave request
    make_request "POST" "$BASE_URL/api/leave/requests/leave-test-2024-12-01/cancel/" "" "Cancel Leave Request"
}
...
test_leave_balances() {
    ...
    make_request "POST" "$BASE_URL/api/leave/balances/" \
        '{"employee": 1, "leave_type": "annual_leave", "year": 2024, "total_days": 25.0, "used_days": 0.0}' \
        "Create Leave Balance"

    # Get specific leave balance
    make_request "GET" "$BASE_URL/api/leave/balances/1/" "" "Get Leave Balance Details"

    # Update leave balance
    make_request "PATCH" "$BASE_URL/api/leave/balances/1/" \
        '{"used_days": 2.5}' \
        "Update Leave Balance"
}
...
test_leave_policies() {
    ...
    make_request "POST" "$BASE_URL/api/leave/policies/" \
        '{"leave_type": "sick_leave", "annual_entitlement": 10.0, "max_consecutive_days": 5, "notice_period_days": 1}' \
        "Create Leave Policy"
...
test_workflows() {
    ...
    make_request "PUT" "$BASE_URL/api/leave/workflows/1/update_configs/" \
        '{"level_configs": [{"level": 1, "approval_type": "role", "required_role": "HR Manager"}]}' \
    ...
    # Send reminders
    make_request "POST" "$BASE_URL/api/leave/workflows/send_reminders/" "" "Send Approval Reminders"

    # Send escalations
    make_request "POST" "$BASE_URL/api/leave/workflows/send_escalations/" "" "Send Escalation Notifications"

    # Quick approve
    make_request "POST" "$BASE_URL/api/leave/workflows/1/quick_approve/" "" "Quick Approve Request"

    # Quick reject
    make_request "POST" "$BASE_URL/api/leave/workflows/1/quick_reject/" \
