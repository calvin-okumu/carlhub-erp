...
class ProjectService:
    ...
    @staticmethod
    def get_projects_for_user(user_tenant: UserTenant, search: Optional[str] = None,
                            ordering: Optional[str] = None, status: Optional[str] = None,
                            client: Optional[str] = None) -> List[Project]:
        """
    ...
    @staticmethod
    def get_project_by_slug(user_tenant: UserTenant, slug: str) -> Optional[Project]:
        """
    ...
    def create_project(user_tenant: UserTenant, project_data: Dict[str, Any]) -> Project:
        ...
        if 'client' in project_data:
            client = Client.objects.filter(tenant=user_tenant.tenant, slug=project_data['client']).first()
            if not client:
    ...
    @staticmethod
    @transaction.atomic
    def update_project(user_tenant: UserTenant, slug: str, update_data: Dict[str, Any]) -> Optional[Project]:
        """
    ...
    def delete_project(user_tenant: UserTenant, slug: str) -> bool:
        ...
        if project.milestones.exists() or project.tasks.exists():
            raise ValidationError("Cannot delete project with associated milestones or tasks. Please delete them first.")
    ...
    @staticmethod
    def duplicate_project(user_tenant: UserTenant, slug: str, new_name: str) -> Optional[Project]:
        """
