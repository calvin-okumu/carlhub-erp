...
from ..models import Client, Invoice, Payment, Project, Tenant
...
class InvoiceService:
    ...
    @staticmethod
    def get_invoice_by_slug(user_tenant: UserTenant, slug: str) -> Invoice | None:
        """
    ...
    def create_invoice(user_tenant: UserTenant, invoice_data: dict[str, Any]) -> Invoice:
        ...
        if "client" in invoice_data:
            client = Client.objects.filter(
                tenant=user_tenant.tenant, slug=invoice_data["client"]
            ).first()
            if not client:
        ...
        if "project" in invoice_data and invoice_data["project"]:
            project = Project.objects.filter(
                tenant=user_tenant.tenant, slug=invoice_data["project"]
            ).first()
            if not project:
        ...
        if not invoice_data.get("invoice_number"):
            invoice_data["invoice_number"] = InvoiceService._generate_invoice_number(
                user_tenant.tenant
            )
    ...
    def update_invoice(
        ...
        if "project" in update_data and update_data["project"]:
            project = Project.objects.filter(
                tenant=user_tenant.tenant, slug=update_data["project"]
            ).first()
            if not project:
                raise ValidationError("Invalid project.")
    ...
    def _generate_invoice_number(tenant: Tenant) -> str:
        ...
        last_invoice = (
            Invoice.objects.filter(tenant=tenant, invoice_number__startswith=prefix)
    ...
    def get_overdue_invoices(user_tenant: UserTenant) -> list[Invoice]:
        ...
        return Invoice.objects.filter(
            tenant=user_tenant.tenant, paid=False, due_date__lt=timezone.now().date()
        ).order_by("due_date")
