    ...
    def has_permission(self, request, view) -> bool:
        ...
        return bool(
            UserTenant.objects.filter(
                user=request.user, tenant=request.tenant, is_approved=True
            ).exists()
        )
    ...
    def has_permission(self, request, view) -> bool:
        ...
        if required_perm:
            ...
            return self._check_permission(
                request.user, required_perm, getattr(request, "tenant", None)
            )
        return False
...
class IsTenantOwner(permissions.BasePermission):
    ...
    def has_permission(self, request, view) -> bool:
        ...
        return bool(
            UserTenant.objects.filter(
                user=request.user, tenant=request.tenant, is_owner=True, is_approved=True
            ).exists()
        )
    ...
    def has_object_permission(self, request, view, obj) -> bool:
        ...
        return bool(
            hasattr(obj, "tenant")
            and obj.tenant == request.tenant
            and UserTenant.objects.filter(
                user=request.user, tenant=request.tenant, is_owner=True, is_approved=True
            ).exists()
        )
...
class IsTenantCreator(permissions.BasePermission):
    ...
    def _user_is_tenant_owner(self, request, tenant):
        ...
        return UserTenant.objects.filter(
            user=request.user, tenant=tenant, is_owner=True, is_approved=True
        ).exists()
