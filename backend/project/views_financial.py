...
class InvoiceViewSet(OptimizedTenantScopedMixin, OptimizedViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if overdue == "true":
            queryset = queryset.filter(status="sent", due_date__lt=timezone.now().date())
    ...
    def overdue_invoices(self, request):
        ...
        overdue_invoices = self.get_queryset().filter(
            status="sent", due_date__lt=timezone.now().date()
        )
    ...
    def bulk_delete_invoices(self, request):
        ...
        if not invoice_ids:
            return Response(
                {"error": "invoice_ids are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for invoice in invoices_to_delete:
            if invoice.payments.exists():
                invoices_with_payments.append(
                    f"Invoice {invoice.id} ({invoice.invoice_number}) has associated payments"
                )
...
@extend_schema_view(
    ...
    create=extend_schema(
        summary="Create payment", description="Create a new payment record for an invoice."
    ),
    update=extend_schema(
...
class PaymentViewSet(OptimizedTenantScopedMixin, OptimizedViewSetMixin, viewsets.ModelViewSet):
    ...
    def bulk_delete_payments(self, request):
        ...
        if not payment_ids:
            return Response(
                {"error": "payment_ids are required"}, status=status.HTTP_400_BAD_REQUEST
            )
