...
from .serializers import (
    CustomUserSerializer, InvitationSerializer, 
    UserTenantSerializer, TenantSerializer
)
...
@extend_schema_view(
    ...
    create=extend_schema(
        summary="Create user-tenant relationship",
...
class UserTenantViewSet(OptimizedTenantScopedMixin, OptimizedViewSetMixin, viewsets.ModelViewSet):
    ...
    def approve_member(self, request, slug=None):
        ...
        if user_tenant.is_approved:
            return Response({'error': 'Member is already approved'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def remove_member(self, request, slug=None):
        ...
        # Send removal email
        try:
            EmailService.send_member_removal_email(
                user_email,
                tenant_name
            )
        except EmailError as e:
...
@extend_schema_view(
    list=extend_schema(
        ...
        description="Retrieve a list of invitations for current tenant."
    ),
    retrieve=extend_schema(
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_superuser:
            ...
            queryset = queryset.filter(
                usertenant__tenant__in=user_tenants
            ).distinct()
