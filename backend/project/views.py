...
@extend_schema_view(
    list=extend_schema(
        summary="List clients", description="Retrieve a list of clients for the current tenant."
    ),
    retrieve=extend_schema(
        summary="Retrieve client", description="Retrieve details of a specific client."
    ),
    create=extend_schema(
        summary="Create client", description="Create a new client for the current tenant."
    ),
    update=extend_schema(
        summary="Update client", description="Update an existing client's information."
    ),
    partial_update=extend_schema(
        summary="Partially update client", description="Partially update a client's information."
    ),
    destroy=extend_schema(summary="Delete client", description="Delete a client."),
)
...
class ClientViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def bulk_delete_clients(self, request):
        ...
        if not client_ids:
            return Response(
                {"error": "client_ids are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for client in clients_to_delete:
            if client.projects.exists():
                clients_with_projects.append(
                    f"Client {client.id} ({client.name}) has associated projects"
                )
...
@extend_schema_view(
    ...
    create=extend_schema(
        summary="Create project", description="Create a new project for the current tenant."
    ),
    update=extend_schema(
        summary="Update project", description="Update an existing project's information."
    ),
    partial_update=extend_schema(
        summary="Partially update project", description="Partially update a project's information."
    ),
...
class ProjectViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related("client").prefetch_related(
            "milestones", "milestones__sprints"
        )
    ...
    def bulk_delete_projects(self, request):
        ...
        if not project_ids:
            return Response(
                {"error": "project_ids are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for project in projects_to_delete:
            if project.invoices.exists():
                projects_with_invoices.append(
                    f"Project {project.id} ({project.name}) has associated invoices"
                )
...
@extend_schema_view(
    ...
    create=extend_schema(
        summary="Create milestone", description="Create a new milestone for a project."
    ),
    update=extend_schema(
        summary="Update milestone", description="Update an existing milestone's information."
    ),
    partial_update=extend_schema(
...
@extend_schema_view(
    list=extend_schema(
        summary="List sprints", description="Retrieve a list of sprints for the current tenant."
    ),
    create=extend_schema(summary="Create sprint", description="Create a new sprint."),
    retrieve=extend_schema(
        summary="Get sprint details",
        description="Retrieve detailed information about a specific sprint.",
    ),
    update=extend_schema(
        summary="Update sprint", description="Update an existing sprint's information."
    ),
    partial_update=extend_schema(
        summary="Partially update sprint", description="Partially update a sprint's information."
    ),
    destroy=extend_schema(
        summary="Delete sprint", description="Delete a sprint and unassign all associated tasks."
    ),
    assign_task=extend_schema(
        summary="Assign task to sprint", description="Assign an existing task to this sprint."
    ),
    unassign_task=extend_schema(
        summary="Unassign task from sprint", description="Remove a task from this sprint."
    ),
)
@extend_schema(
    parameters=[
...
class SprintViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def assign_task(self, request, slug=None):
        ...
        try:
            ...
            return Response(
                {"error": "Task not found or does not belong to the same milestone as the sprint"},
                status=status.HTTP_404_NOT_FOUND,
            )
        except Exception as e:
            logger.error(f"Error assigning task {task_id} to sprint {sprint.id}: {e}")
            return Response(
                {"error": "Failed to assign task"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def bulk_update_sprints(self, request):
        ...
        if not sprint_ids or not new_status:
            return Response(
                {"error": "sprint_ids and status are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for sprint in sprints_to_update:
            if new_status == "completed" and sprint.status != "active":
                invalid_transitions.append(
                    f"Sprint {sprint.id} ({sprint.name}) must be active to complete"
                )
            elif new_status == "active" and sprint.status != "planned":
                invalid_transitions.append(
                    f"Sprint {sprint.id} ({sprint.name}) must be planned to activate"
                )

        if invalid_transitions:
            return Response(
                {"error": "Invalid status transitions", "details": invalid_transitions},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
@extend_schema_view(
    ...
    create=extend_schema(summary="Create task", description="Create a new task for a milestone."),
    update=extend_schema(
        summary="Update task", description="Update an existing task's information."
    ),
    partial_update=extend_schema(
        summary="Partially update task", description="Partially update a task's information."
    ),
...
class TaskViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = (
            super().get_queryset().select_related("milestone", "sprint", "milestone__project")
        )
        ...
        if sprint_id:
            try:
                sprint = Sprint.objects.get(
                    id=sprint_id,
                    tenant=(
                        request.tenant if hasattr(request, "tenant") and request.tenant else None
                    ),
                )
                ...
                if len(task_milestones) > 1 or sprint.milestone.id not in task_milestones:
                    return Response(
                        {
                            "error": "All tasks must belong to the same milestone as the target sprint"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
    ...
    def restore(self, request, slug=None):
        ...
        if hasattr(request, "tenant") and request.tenant and task.tenant != request.tenant:
            return Response(
                {"error": "Task does not belong to your tenant"}, status=status.HTTP_403_FORBIDDEN
            )
...
@extend_schema_view(
    ...
    create=extend_schema(summary="Add tenant member", description="Add a new user to the tenant."),
    update=extend_schema(
        summary="Update tenant member", description="Update a tenant member's information."
    ),
    partial_update=extend_schema(
    ...
    destroy=extend_schema(
        summary="Remove tenant member", description="Remove a user from the tenant."
    ),
)
class UserTenantViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    queryset = UserTenant.objects.select_related("user", "tenant").all()
    ...
    ordering_fields = ["role", "user__email"]
...
@extend_schema_view(
    ...
    update=extend_schema(
        summary="Update invitation", description="Update an existing invitation's information."
    ),
    partial_update=extend_schema(
...
class InvitationViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        tenant = (
            self.request.tenant if hasattr(self.request, "tenant") and self.request.tenant else None
        )
        ...
        if invitation_token:
            try:
                invitation = Invitation.objects.get(
                    token=invitation_token, is_used=False, expires_at__gt=timezone.now()
                )
                if not invitation.email_confirmed:
                    return Response(
                        {
                            "error": "Please confirm your invitation by clicking the link in your email before signing up"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            ...
            if not company_name:
                return Response(
                    {"error": "Company name required for new tenant"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if Tenant.objects.filter(domain=domain).exists():
                return Response(
                    {"error": "Domain already in use"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if website:
                ...
                try:
                    ...
                    return Response(
                        {"error": "Invalid website URL"}, status=status.HTTP_400_BAD_REQUEST
                    )
...
def approve_member_view(request):
    ...
    try:
        ...
        try:
            member_user_tenant = UserTenant.objects.get(
                user_id=user_id, tenant=request.tenant, is_owner=False, is_approved=False
            )
        except UserTenant.DoesNotExist:
        ...
        if member_user_tenant.role == "Tenant Owner":
            ...
            current_owner_count = UserTenant.objects.filter(
                tenant=request.tenant, is_owner=True
            ).count()
            if current_owner_count >= 1:  # Allow multiple owners but ensure at least one
                ...
                return Response(
                    {
                        "error": "Cannot approve member as owner: tenant must maintain at least one owner"
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        try:
            AuditLogger.log_member_approved(
                member_user_tenant, request.user, ip_address=get_client_ip(request)
            )
        except Exception as e:
    ...
    try:
        ...
        if hasattr(request, "tenant") and request.tenant:
            ...
            try:
                ...
                return Response(
                    {"error": "No tenant ownership found"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        try:
            ...
            return Response(
                {"error": "Only owners can invite members"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        if Invitation.objects.filter(email=email, tenant=tenant, is_used=False).exists():
            return Response(
                {"error": "An invitation is already pending for this email in this tenant"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"error": e.user_message, "error_category": e.category, "retryable": e.retryable},
            ...
            logger.error(
                f"Unexpected error sending invitation email to {email}: {error_info['category']} - {str(e)}"
            )
...
@extend_schema(
    ...
    request={
        "application/json": {
            "type": "object",
            "properties": {"token": {"type": "string", "description": "Invitation token"}},
            "required": ["token"],
        }
    },
    parameters=[
...
def confirm_invitation_view(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": "Invalid or expired invitation token"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def resend_invitation_view(request):
    ...
    try:
        ...
        if invitation.email_confirmed:
            return Response(
                {"error": "Invitation already confirmed. Please proceed to signup."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            # Handle our custom email errors with enhanced information
            logger.error(
                f"Email service error resending invitation to {invitation.email}: {e.category} - {str(e)}"
            )
            ...
            error_info = EmailService.classify_email_error(e)
            logger.error(
                f"Unexpected error resending invitation email to {invitation.email}: {error_info['category']} - {str(e)}"
            )
    ...
    request={
        "application/json": {
            ...
            "properties": {
                "user_id": {
                    "type": "integer",
                    "description": "ID of the user to assign/remove admin role",
                },
                "assign_admin": {
                    "type": "boolean",
                    "description": "True to assign admin role, False to remove",
                },
            },
...
def assign_admin_view(request):
    ...
    try:
        ...
        return Response(
            {"error": "Only owners can assign admin roles"}, status=status.HTTP_403_FORBIDDEN
        )
    ...
    if user_id is None or assign_admin is None:
        return Response(
            {"error": "user_id and assign_admin are required"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        ...
        return Response(
            {"error": "User is not an approved member of this tenant"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if not assign_admin and user_tenant.is_owner:
        ...
        if owner_count <= 1:
            return Response(
                {"error": "Cannot remove admin role: tenant must have at least one owner"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    try:
        AuditLogger.log(
            ...
            old_values={
                "is_owner": not assign_admin,
                "role": "Employee" if assign_admin else "Tenant Owner",
            },
...
def transfer_ownership_view(request):
    ...
    try:
        ...
        return Response(
            {"error": "Only owners can transfer ownership"}, status=status.HTTP_403_FORBIDDEN
        )
    ...
    try:
        from_user_tenant, to_user_tenant = UserTenant.transfer_ownership(
            tenant=tenant, from_user=request.user, to_user=to_user
        )
...
def health_check(request):
    ...
    """
    return Response(
        {"status": "healthy", "timestamp": timezone.now().isoformat(), "service": "DjangoCRM API"},
        status=200,
    )
...
def excel_export_view(request):
    ...
    if not model_type:
        return Response(
            {"error": "Model parameter is required"}, status=status.HTTP_400_BAD_REQUEST
        )

    if model_type not in ["clients", "projects", "tasks"]:
        return Response(
            {"error": "Invalid model type. Must be one of: clients, projects, tasks"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        ...
        if tenant is None:
            ...
            try:
                ...
                if user_tenant:
                    ...
                    return Response(
                        {"error": "No tenant access found"}, status=status.HTTP_403_FORBIDDEN
                    )
            except UserTenant.DoesNotExist:
                return Response(
                    {"error": "No tenant access found"}, status=status.HTTP_403_FORBIDDEN
        ...
        return Response(
            {"error": "Failed to export data"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def excel_import_view(request):
    ...
    try:
        ...
        return Response(
            {"error": "Failed to import data"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
