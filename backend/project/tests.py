...
class ExcelExportImportTests(APITestCase):
    ...
    def setUp(self):
        # Create user with proper permissions
        self.user = CustomUser.objects.create_user(email='exceltest@example.com', password='testpass')
        ...
        permissions = Permission.objects.filter(content_type__in=content_types, codename__in=[
            'view_client', 'view_project', 'view_task'
        ])
        self.user.user_permissions.add(*permissions)
        self.client.force_authenticate(user=self.user)
    ...
    def test_project_export(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response['Content-Type'], 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        self.assertIn('attachment; filename="projects_export.xlsx"', response['Content-Disposition'])
        # Check that response has content
        self.assertGreater(len(response.content), 0)
