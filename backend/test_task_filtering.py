...
def login():
    ...
    response = requests.post(
        f"{BASE_URL}/api/login/", json={"email": "test@example.com", "password": "password123"}
    )
...
def test_query_parameter():
    ...
    filtered_response = requests.get(f"{BASE_URL}/api/tasks/?sprint={sprint_slug}", headers=headers)
...
def test_nested_route():
    ...
    sprints_response = requests.get(
        f"{BASE_URL}/api/projects/{project_slug}/sprints/", headers=headers
    )
    if sprints_response.status_code != 200 or sprints_response.json()["count"] == 0:
    ...
    nested_response = requests.get(
        f"{BASE_URL}/api/projects/{project_slug}/sprints/{sprint_slug}/tasks/", headers=headers
    )
