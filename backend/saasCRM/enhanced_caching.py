...
class CacheManager:
    ...
    @classmethod
    def set(cls, key: str, value: Any, timeout: int | None = None) -> bool:
        ...
        try:
            ...
            if hasattr(cache_backend, "_cache"):
                ...
                if settings.DEBUG and keys_to_delete:
                    logger.debug(
                        f"Cache DELETE_PATTERN_FALLBACK: {pattern} -> {len(keys_to_delete)} keys"
                    )
    ...
    @classmethod
    def invalidate_model(cls, model_name: str, tenant_id: str | None = None):
    ...
    @classmethod
    def cache_object(cls, instance: models.Model, timeout: int | None = None):
    ...
    @classmethod
    def get_cached_object(cls, model_class: models.Model, object_id: int) -> dict | None:
    ...
    def cache_queryset(
        ...
        key = cls.get_key(
            "list", model=model_name, tenant=tenant_id or "default", filters_hash=filters_hash
        )
    ...
    def get_cached_queryset(
        cls, model_name: str, tenant_id: str | None, filters: dict[str, Any]
    ) -> dict | None:
...
class CacheDecorator:
    ...
    @staticmethod
    def cache_result(timeout: int = 300, key_prefix: str = "", vary_on: list[str] | None = None):
        """
...
def cache_user_permissions(
    user_id: int, tenant_id: int, permissions: list[str], timeout: int | None = None
):
...
def get_cached_user_permissions(user_id: int, tenant_id: int) -> list[str] | None:
    """Get cached user permissions for a tenant."""
    key = CacheManager.get_key("user_permissions", user_id=user_id, tenant_id=tenant_id)
    return CacheManager.get(key)


def cache_tenant_data(tenant_id: int, data: dict[str, Any], timeout: int | None = None):
...
def get_cached_tenant_data(tenant_id: int) -> dict[str, Any] | None:
