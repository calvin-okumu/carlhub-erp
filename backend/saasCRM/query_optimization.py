...
from project.models import Milestone, Project, Sprint, Task
...
class OptimizedTenantScopedMixin(OptimizedQuerySetMixin):
    ...
    def get_queryset(self):
        ...
        if hasattr(self.request, "tenant") and self.request.tenant:
            ...
            user_tenants = (
                UserTenant.objects.filter(user=self.request.user)
...
def get_optimized_queryset(
    model_class, tenant=None, user=None, select_fields=None, prefetch_fields=None
):
    """
...
def create_prefetch_for_tasks():
    ...
    return Prefetch(
        "tasks",
        queryset=(
            Task.objects.select_related("assignee", "milestone", "sprint").order_by("created_at")
        ),
    )
...
def create_prefetch_for_sprints():
    ...
    return Prefetch(
        "sprints",
        queryset=(
            Sprint.objects.select_related("milestone")
...
def create_prefetch_for_milestones():
    ...
    return Prefetch(
        "milestones",
        queryset=(
            Milestone.objects.select_related("assignee", "project")
...
def create_prefetch_for_projects():
    ...
    return Prefetch(
        "projects",
        queryset=(
            Project.objects.select_related("client", "tenant")
            .prefetch_related("team_members", "access_groups", create_prefetch_for_milestones())
            .order_by("created_at")
        ),
    )
