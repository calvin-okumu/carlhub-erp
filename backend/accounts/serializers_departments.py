...
from .models import Department, UserTenant, CustomUser
...
class DepartmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Department
        fields = [
            'id', 'slug', 'name', 'description', 'manager', 'manager_name',
            'employee_count', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'slug', 'created_at', 'updated_at', 'employee_count', 'manager_name']
...
class DepartmentDetailSerializer(DepartmentSerializer):
    ...
    def get_employees(self, obj):
        ...
        user_tenants = UserTenant.objects.filter(department=obj, is_approved=True).select_related('user')
        return [
            {
                'id': ut.user.id,
                'email': ut.user.email,
...
class DepartmentCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        if queryset.exists():
            raise serializers.ValidationError("Department with this name already exists in this tenant")
