...
class InvitationModelTests(TestCase):
    ...
    def setUp(self):
        ...
        UserTenant.objects.create(
            user=self.invited_by, tenant=self.tenant, is_owner=True, is_approved=True
        )
...
class InvitationAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.owner = CustomUser.objects.create_user(
            email="owner@example.com", password="testpass123", first_name="Owner", last_name="User"
        )
        UserTenant.objects.create(
    ...
    def test_invite_member(self):
        ...
        data = {"email": "newmember@example.com", "role": "Employee"}
    ...
    def test_invite_existing_user_from_different_tenant_succeeds(self):
        ...
        UserTenant.objects.create(
            user=existing_user, tenant=other_tenant, is_owner=True, is_approved=True
        )
    ...
    def test_invite_user_already_in_same_tenant_fails(self):
        ...
        UserTenant.objects.create(user=existing_member, tenant=self.tenant, is_approved=True)
    ...
    def test_invite_user_with_pending_invitation_fails(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["error"], "An invitation is already pending for this email in this tenant"
        )
