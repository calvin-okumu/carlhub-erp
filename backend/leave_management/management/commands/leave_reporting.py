...
from leave_management.models import LeaveBalance, LeaveRequest, LeavePolicy
...
class Command(BaseCommand):
    ...
    def _summary_report(self, tenant, year):
        ...
        total_remaining = balances.aggregate(
            total=Sum('total_days') + Sum('carried_over') - Sum('used_days')
        )['total'] or 0
        ...
        total_days_requested = requests.filter(status='approved').aggregate(
    ...
    def _detailed_report(self, tenant, year):
        ...
        self.stdout.write(f"{'Employee':<30} {'Type':<12} {'Entitlement':<12} {'Used':<8} {'Carried':<10} {'Remaining':<10} {'Utilization':<12}")
    ...
    def _compliance_report(self, tenant, year):
        ...
        for policy in policies:
            ...
            self.stdout.write(f"  Carry Over Allowed: {'Yes' if policy.carry_over_allowed else 'No'}")
            ...
            if violations:
                self.stdout.write(f"  Policy Violations:")
                for violation in violations:
                    self.stdout.write(f"    - {violation}")
            else:
                self.stdout.write(f"  Policy Violations: None")

        # Check for employees without balances
        from accounts.models import UserTenant
        employees_with_balances = LeaveBalance.objects.filter(
            tenant=tenant, year=year
        ).values_list('employee', flat=True).distinct()

        all_employees = UserTenant.objects.filter(
            tenant=tenant, is_approved=True
