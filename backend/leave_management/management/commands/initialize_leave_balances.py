    ...
    def _process_tenant(self, tenant, year, dry_run):
        ...
        for employee in employees:
            for policy in policies:
                balance, created = self._ensure_leave_balance(
                    employee.user, policy, year, dry_run
                )
                if created:
                    created_count += 1
                    if not dry_run:
                        ...
                        self.stdout.write(f"    ✓ Would create: {employee.user.get_full_name()} - {policy.leave_type} ({year})")
                else:
                    skipped_count += 1
