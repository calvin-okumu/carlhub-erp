...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--pattern", type=str, help="Cache pattern to invalidate (used with invalidate action)"
        )
        parser.add_argument(
            "--model", type=str, help="Model name to warm up cache for (used with warm action)"
        )
        parser.add_argument(
            "--tenant-id", type=int, help="Tenant ID to warm up cache for (used with warm action)"
        )
    ...
    def warm_cache(self, model=None, tenant_id=None):
        ...
        if not model:
            self.stdout.write(self.style.WARNING("Please specify --model to warm up cache for"))
            return
        ...
        try:
            ...
            if model == "client":
                ...
                self.stdout.write(self.style.SUCCESS(f"✓ Cached {queryset.count()} client objects"))
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Cached {queryset.count()} project objects")
                )
                ...
                for user in queryset.prefetch_related("usertenant_set"):
                    ...
                    for ut in user.usertenant_set.all():
                        ...
                        CacheManager.cache_user_permissions(user.id, ut.tenant_id, permissions)

                self.stdout.write(
                    self.style.SUCCESS(f"✓ Cached {queryset.count()} user objects and permissions")
                )

            else:
                self.stdout.write(self.style.WARNING(f'Model "{model}" not supported for warming'))
    ...
    def invalidate_pattern(self, pattern=None):
        ...
        if not pattern:
            self.stdout.write(self.style.WARNING("Please specify --pattern to invalidate"))
            return
