...
class TenantContextResolutionTests(APITestCase):
    ...
    def setUp(self):
        ...
        UserTenant.objects.create(
            user=self.owner, tenant=self.tenant, is_owner=True, is_approved=True
        )
    ...
    def test_approved_user_can_create_leave_request(self):
        ...
        approved_user = CustomUser.objects.create_user(
            username="approved", email="approved@example.com", password="testpass123"
        )
        UserTenant.objects.create(user=approved_user, tenant=self.tenant, is_approved=True)
    ...
    def test_unapproved_user_cannot_create_leave_request(self):
        ...
        unapproved_user = CustomUser.objects.create_user(
            username="unapproved", email="unapproved@example.com", password="testpass123"
        )
        UserTenant.objects.create(user=unapproved_user, tenant=self.tenant, is_approved=False)
    ...
    def test_user_with_expired_invitation_cannot_create_leave_request(self):
        ...
        expired_user = CustomUser.objects.create_user(
            username="expired", email="expired@example.com", password="testpass123"
        )
    ...
    def test_user_with_no_tenant_membership_cannot_create_leave_request(self):
        ...
        orphan_user = CustomUser.objects.create_user(
            username="orphan", email="orphan@example.com", password="testpass123"
        )
    ...
    def test_multiple_pending_invitations_error_message(self):
        ...
        tenant2 = Tenant.objects.create(name="Second Company", domain="secondcompany.com")

        # Create user with multiple invitations
        multi_user = CustomUser.objects.create_user(
            username="multi", email="multi@example.com", password="testpass123"
        )
