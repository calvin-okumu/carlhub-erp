...
class PermissionManagementTestCase(APITestCase):
    def setUp(self):
        ...
        UserTenant.objects.create(user=self.regular_user, tenant=self.tenant, is_approved=True)
    ...
    def test_create_permission_group(self):
        ...
        data = {"name": "Test Group", "description": "A test permission group"}

        response = self.client.post("/api/accounts/permission-groups/", data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            PermissionGroup.objects.filter(name="Test Group", tenant=self.tenant).exists()
        )
    ...
    def test_assign_permissions_to_group(self):
        ...
        # Create group
        group = PermissionGroup.objects.create(name="Test Group", tenant=self.tenant)
    ...
    def test_permission_isolation_between_tenants(self):
        ...
        other_permission = CustomPermission.objects.create(
            ...
            app_label=f"tenant_{other_tenant.id}",
        )
...
class CustomPermissionModelTestCase(TestCase):
    ...
    def test_permission_group_slug_generation(self):
        group = PermissionGroup.objects.create(
            name="Test Group With Spaces", tenant=self.tenant, created_by=self.user
        )
