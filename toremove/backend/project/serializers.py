...
class ClientSerializer(serializers.ModelSerializer):
    ...
    projects_count = serializers.SerializerMethodField(
        help_text="Number of projects associated with this client"
    )
...
class ProjectSerializer(serializers.ModelSerializer):
    client_name = serializers.CharField(
        source="client.name", read_only=True, help_text="Name of the associated client"
    )
    milestones_count = serializers.SerializerMethodField(
        help_text="Number of milestones in this project"
    )
...
class MilestoneSerializer(serializers.ModelSerializer):
    project_name = serializers.CharField(
        source="project.name", read_only=True, help_text="Name of the parent project"
    )
    sprints_count = serializers.SerializerMethodField(
        help_text="Number of sprints in this milestone"
    )
    progress = serializers.IntegerField(
        min_value=0, max_value=100, help_text="Milestone progress percentage (0-100)"
    )
...
class SprintSerializer(serializers.ModelSerializer):
    milestone_name = serializers.CharField(
        source="milestone.name", read_only=True, help_text="Name of the parent milestone"
    )
    tasks_count = serializers.SerializerMethodField(help_text="Number of tasks in this sprint")
    progress = serializers.IntegerField(
        min_value=0, max_value=100, read_only=True, help_text="Sprint progress percentage (0-100)"
    )
        ...
        if value == "completed":
            ...
            if instance and not all(task.status == "done" for task in instance.tasks.all()):
                raise serializers.ValidationError(
                    "Cannot mark sprint as completed until all tasks are done."
                )
        return value
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    sprint_name = serializers.CharField(
        source="sprint.name", read_only=True, help_text="Name of the assigned sprint (if any)"
    )
    progress = serializers.IntegerField(
        min_value=0, max_value=100, read_only=True, help_text="Task progress percentage (0-100)"
    )
    is_assigned = serializers.SerializerMethodField(
        help_text="Whether the task is assigned to a sprint"
    )
    milestone = serializers.CharField(help_text="Milestone slug")
    sprint = serializers.CharField(
        required=False, allow_null=True, help_text="Sprint slug (optional)"
    )
    ...
    def validate(self, attrs):
        ...
        from .models import Milestone, Sprint
...
class UserTenantSerializer(serializers.ModelSerializer):
    ...
    user_first_name = serializers.CharField(
        source="user.first_name", read_only=True, help_text="First name of the user"
    )
    user_last_name = serializers.CharField(
        source="user.last_name", read_only=True, help_text="Last name of the user"
    )
    tenant_name = serializers.CharField(
        source="tenant.name", read_only=True, help_text="Name of the tenant organization"
    )
...
class InvitationSerializer(serializers.ModelSerializer):
    ...
    invited_by_email = serializers.CharField(
        ...
        read_only=True,
        help_text="Email of the user who sent the invitation",
    )
