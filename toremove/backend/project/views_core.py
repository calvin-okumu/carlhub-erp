...
@extend_schema_view(
    ...
    destroy=extend_schema(
        summary="Delete client", description="Delete a client from the current tenant."
    ),
)
...
@extend_schema_view(
    list=extend_schema(
        summary="List sprints", description="Retrieve a list of sprints for current tenant."
    ),
...
class SprintViewSet(OptimizedTenantScopedMixin, OptimizedViewSetMixin, viewsets.ModelViewSet):
    ...
    def assign_task(self, request, slug=None):
        ...
        try:
            ...
            return Response(
                {"error": "Task not found or does not belong to same milestone as sprint"},
        ...
        if sprint_id:
            try:
                ...
                if len(task_milestones) > 1 or sprint.milestone.id not in task_milestones:
                    return Response(
                        {"error": "All tasks must belong to same milestone as target sprint"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
