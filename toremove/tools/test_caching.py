...
from saasCRM.enhanced_caching import CacheDecorator, CacheManager, CacheStats
...
def test_model_caching():
    ...
    try:
        ...
        tenant, _ = Tenant.objects.get_or_create(
            name="Test Tenant", defaults={"domain": "test.com"}
        )
        ...
        client, _ = Client.objects.get_or_create(
            name="Test Client", defaults={"tenant": tenant}
        )
        ...
        print(
            f"Retrieved cached client data: {cached_data['data']['name'] if cached_data else 'None'}"
        )

        # Test queryset caching
        filters = {"tenant": tenant.id}
        queryset_data = [{"id": client.id, "name": client.name}]
        CacheManager.cache_queryset(
            "client", str(tenant.id), filters, queryset_data, timeout=60
        )

        cached_queryset = CacheManager.get_cached_queryset(
            "client", str(tenant.id), filters
        )
        print(
            f"Cached queryset count: {cached_queryset['count'] if cached_queryset else 0}"
        )
